...
#
# This module provides the high-level Camera class that handles all
...
from .constants import Opcode, Protocol, EventType, ChannelIOCTL
from .exceptions import OMVException, TimeoutException, ResyncException
...
class Camera:
    ...
    def _resync(self):
        ...
        self.transport = Transport(self._serial, crc=True, seq=True,
    ...
    def _channel_lock(self, channel_id):
        """Lock a data channel"""
        return self._send_cmd_wait_resp(Opcode.CHANNEL_LOCK, channel_id)

    def _channel_unlock(self, channel_id):
        """Lock a data channel"""
        return self._send_cmd_wait_resp(Opcode.CHANNEL_UNLOCK, channel_id)

    def _channel_size(self, channel_id):
        """Get available data size for a channel"""
        payload = self._send_cmd_wait_resp(Opcode.CHANNEL_SIZE, channel_id)
        return struct.unpack('<I', payload[:4])[0]

    def _channel_shape(self, channel_id):
        """Get available data size for a channel"""
        payload = self._send_cmd_wait_resp(Opcode.CHANNEL_SHAPE, channel_id)
    ...
    def _channel_read(self, channel_id, offset, length):
        ...
        data = self._send_cmd_wait_resp(Opcode.CHANNEL_READ, channel_id, payload)
        return bytes(data)
    ...
    def _channel_write(self, channel_id, data, offset=0):
        ...
        for start in range(0, len(data), chunk_size):
            ...
            self._send_cmd_wait_resp(Opcode.CHANNEL_WRITE, channel_id, payload)
    ...
    def _channel_ioctl(self, channel_id, cmd, fmt=None, *args):
        ...
        return self._send_cmd_wait_resp(Opcode.CHANNEL_IOCTL, channel_id, payload)
    ...
    def _channel_list(self):
        ...
        payload = self._send_cmd_wait_resp(Opcode.CHANNEL_LIST)
    ...
    def connect(self):
        ...
        try:
            # Enable hw flow control with rtscts=True
            ...
            self.disconnect()
            raise OMVException(f"Failed to connect: {e}")
    ...
    def device_stats(self):
        ...
        payload = self._send_cmd_wait_resp(Opcode.PROTO_STATS)

        if len(payload) < 32:
            raise OMVException(f"Invalid PROTO_STATS payload size: {len(payload)}")
    ...
    def reset(self):
        """Reset the camera"""
        self._send_cmd_wait_resp(Opcode.SYS_RESET)

    @retry_if_failed
    def boot(self):
        """Jump to bootloader"""
        self._send_cmd_wait_resp(Opcode.SYS_BOOT)

    @retry_if_failed
    def update_capabilities(self):
        """Set device capabilities"""
        payload = self._send_cmd_wait_resp(Opcode.PROTO_GET_CAPS)
        ...
        self._send_cmd_wait_resp(Opcode.PROTO_SET_CAPS, 0, payload)
    ...
    def stop(self):
        ...
        self._channel_ioctl(stdin_id, ChannelIOCTL.STDIN_STOP)
    ...
    def exec(self, script):
        ...
        stdin_id = self.get_channel(name="stdin")
        # Reset script buffer
        self._channel_ioctl(stdin_id, ChannelIOCTL.STDIN_RESET)
        # Upload script data
    ...
    def read_status(self):
        ...
        payload = self._send_cmd_wait_resp(Opcode.CHANNEL_POLL)
    ...
    @retry_if_failed
    def profiler_reset(self, config=None):
        """Reset the profiler data"""
        if profile_id := self.get_channel(name="profile"):
            self._channel_ioctl(profile_id, ChannelIOCTL.PROFILE_RESET)
            logging.debug("Profiler reset")

        if config is None:
            # CPU cycles, and L1I, L1D, L2D cache info
            events = [0x0039, 0x0023, 0x0024, 0x0001, 0x0003, 0xC102, 0x02CC, 0xC303]
            for e in zip(range(len(events)), events):
                self.profiler_event(*e)
    ...
    def profiler_mode(self, exclusive=False):
        ...
        if profile_id := self.get_channel(name="profile"):
            ...
            self._channel_ioctl(profile_id, ChannelIOCTL.PROFILE_MODE, 'I', mode)
            logging.debug(f"Profile mode set to {'exclusive' if exclusive else 'inclusive'}")

    @retry_if_failed
    def profiler_event(self, counter_num, event_id):
        """Configure an event counter to monitor a specific event"""
        if profile_id := self.get_channel(name="profile"):
            self._channel_ioctl(profile_id, ChannelIOCTL.PROFILE_SET_EVENT, 'II', counter_num, event_id)
    ...
    def print_system_info(self):
        ...
        # USB VID/PID
        logging.info(f"USB ID: {self.sysinfo['usb_vid']:04X}:{self.sysinfo['usb_pid']:04X}")
